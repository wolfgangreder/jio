...
public class EnumerateChips {
  ...
  private static Predicate<ChipId> filterChipIds(boolean showAll)
  {
    return chip -> showAll || chip.isAccesible();
  }

  public static boolean run(CommandLine commandLine, Io lg) throws IOException
  {
    final boolean showAll = commandLine.hasOption("show-all");
    List<ChipId> foundChips = lg.enumerateChips()
            .map(chipId -> {
