...
public class ShowChipInfo {
  ...
  private static List<ChipId> getRequiredChipIds(CommandLine cmdLine, Io lgIo) throws IOException
  {
    Predicate<ChipId> filter = (ChipId chipId) -> chipId.isAccesible();
    List<String> values = extractValues(cmdLine);
    if (!values.isEmpty()) {
      Set<ChipId> requiredChips = new HashSet<>();

      for (String value : values) {
        ChipId newId;
        if (ChipId.matchDeviceName(value).matches()) {
          newId = new ChipId(value);
        } else {
          try {
  ...
  public static boolean showChipInfo(CommandLine cmdLine, Io lgIo) throws IOException
  {
    List<ChipId> chipIds = getRequiredChipIds(cmdLine, lgIo);
    if (!chipIds.isEmpty()) {
      for (ChipId id : chipIds) {
        try (IoChip chip = lgIo.open(id)) {
          System.out.println("ChipInfo for " + chip.getId());
          ChipInfo info = chip.getChipInfo();
